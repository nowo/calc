...
| `rounding` | `'truncate'`(默认) / `'halfUp'` / `'banker'` / `'ceil'`(→+∞) / `'floor'`(→−∞) / `'expand'`(远离零) —— 含 JS 别名 `trunc`/`round`/`halfEven` |
...
| `add/sub/mul/div/sqrt/pow/mod`（→ number）、`addStr/…/sqrtStr/powStr/modStr`（→ string） | 独立运算 |
| `calcSum/calcAvg/calcMedian/calcMax/calcMin` | 数组 / 对象数组聚合 |
